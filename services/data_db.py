...
class PerformanceEstimator(SQLModel, table=True):
    __tablename__ = "performance_estimator_plus"
...
class CommitmentAnswer(SQLModel, table=True):
    ...
    date: date
    patient: Optional[Patient] = Relationship(back_populates="commitment_answers")
...
if __name__ == "__main__":
    ...
    #     # Get patient's sessions through prescriptions
    #     patient = session.get(Patient, 2607)
    #     print(patient)

        # patient_agg = ProtocolSessions(patient_id=patient.patient_id)

        # for p in patient.prescriptions:
        #     for s in p.sessions:s
        #         s.protocol_id = p.protocol_id

        # patient_agg.add_sessions(np.concat([p.sessions for p in patient.prescriptions]))
        # print(patient.prescriptions[0].sessions[0].adherence)
        # emotional_answers = session.exec(select(EmotionalAnswer)).all()
        # for answer in emotional_answers:
        #     print(f"Emotional Answer ID: {answer.id}, Patient ID: {answer.patient_id}")